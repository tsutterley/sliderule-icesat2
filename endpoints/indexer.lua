...
userlog:sendlog(core.INFO, string.format("atl03 indexing initiated on %s data...", atl03_asset))
...
while not indexer:waiton(interval) do
    ...
    if timeout > 0 and duration == timeout then
        userlog:sendlog(core.INFO, string.format("request timed-out after %d seconds", duration / 1000))
        return
    end
    -- Get Stats --
    local stats = indexer:stats()
    -- Dispay Progress --
    userlog:sendlog(core.INFO, string.format("processed %d entries from %d threads", stats.processed, stats.threads))
end
